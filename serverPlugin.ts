...
export const vuePlugin: ServerPlugin = ({
	...
	app.use(async (ctx, next) => {
		if(ctx.path === '/vite/hmr'){
			await next()
			ctx.type = 'js'
			ctx.body = ctx.body.replace(/__VUE_HMR_RUNTIME__\.rerender\(path, (.+)\)/g, '__VUE_HMR_RUNTIME__.rerender(path, m)')
			return
		}
...
async function parseSFC(
	...
	if (descriptor.script) {
		const { code } = await transform(descriptor.script.content, publicPath, {loader: 'ts'})
		 ...
		 if (!__VUE_HMR_RUNTIME__.isRecorded('${publicPath}')) {
			 __VUE_HMR_RUNTIME__.createRecord('${publicPath}', component.options)
		 	 // import.meta.hot.acceptDeps("${publicPath}?type=template", (mod) => {
		 		...
		 		// 	 __VUE_HMR_RUNTIME__.${hasFunctional ? 'rerender' : 'reload'}('${publicPath}', component.options)
		 	 // })
		 }
	} else {
