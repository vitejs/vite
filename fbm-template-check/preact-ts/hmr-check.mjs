// Standalone HMR workflow for the create-vite `preact-ts` template under Vite full-bundle mode.
...
//
// preact uses @preact/preset-vite (prefresh) HMR, so the correct outcome is a COMPONENT HOT-SWAP
// with STATE PRESERVED (per GOAL.md §1):
...
async function run() {
  ...
  // preact => component HOT-SWAP with STATE PRESERVED. The new <h1> text must appear via a hot
  ...
  if (reloaded)
    return fail(
      'expected a Preact component hot-swap, but the page fully reloaded (window.__noReload lost)',
    )
  if (!statePreserved)
    return fail(
  ...
  console.log(
    `PASS ${TAG}: preact-ts hot-swapped src/app.tsx with counter state preserved.`,
  )
  await cleanup(0)
}
