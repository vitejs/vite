// Standalone HMR workflow for the create-vite `react` template under Vite full-bundle mode.
...
//
// react uses @vitejs/plugin-react's React Fast Refresh, so the correct outcome is a COMPONENT
...
async function run() {
  ...
  // react => component HOT-SWAP with STATE PRESERVED via React Fast Refresh. The new <h1> text
  // must appear via a hot update (NOT a full reload), and the counter must still read "Count is 1".
  if (hmrErrors.length)
  ...
  console.log(
    `PASS ${TAG}: react Fast Refresh hot-swapped App.jsx with counter state preserved.`,
  )
  await cleanup(0)
}
