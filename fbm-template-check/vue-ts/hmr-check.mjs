// Standalone HMR workflow for the create-vite `vue-ts` template under Vite full-bundle mode.
// No Vite test harness: we spawn the app's own `vite` dev server, drive it with Playwright,
// edit src/components/HelloWorld.vue (which renders the <h1> marker + the counter button),
// and assert the hot update.
//
// vue-ts uses @vitejs/plugin-vue HMR, so the correct outcome is a COMPONENT HOT-SWAP with
// STATE PRESERVED (per GOAL.md §1):
...
const EDIT = join(__dirname, 'src/components/HelloWorld.vue') // renders the <h1> marker + counter
...
async function run() {
  ...
  // vue-ts => component HOT-SWAP with STATE PRESERVED. The new <h1> text must appear via a hot
  ...
  console.log(
    `PASS ${TAG}: vue-ts hot-swapped HelloWorld.vue with counter state preserved.`,
  )
  await cleanup(0)
}
