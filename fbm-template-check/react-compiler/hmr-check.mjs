// Standalone HMR workflow for the create-vite `react-compiler` template under Vite full-bundle mode.
...
//
// react-compiler is an OFFLINE BUILT-IN: it scaffolds template-react and runs setupReactCompiler,
// which adds babel-plugin-react-compiler via @rolldown/plugin-babel + reactCompilerPreset() alongside
// the @vitejs/plugin-react plugin. HMR is React Fast Refresh, so the correct outcome is a COMPONENT
// HOT-SWAP with STATE PRESERVED (per GOAL.md §1):
...
async function run() {
  ...
  // react-compiler => React Fast Refresh COMPONENT HOT-SWAP with STATE PRESERVED. The new <h1>
  // text must appear via a hot update (NOT a full reload), and the counter must still read "Count is 1".
  if (hmrErrors.length)
  ...
  console.log(
    `PASS ${TAG}: react-compiler hot-swapped App.jsx with counter state preserved.`,
  )
  await cleanup(0)
}
