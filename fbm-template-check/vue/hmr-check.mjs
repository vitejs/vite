// Standalone HMR workflow for the create-vite `vue` template under Vite full-bundle mode.
...
//
// vue uses @vitejs/plugin-vue HMR, so the correct outcome is a COMPONENT HOT-SWAP with
...
async function run() {
  ...
  console.log(
    `PASS ${TAG}: vue hot-swapped HelloWorld.vue with counter state preserved.`,
  )
  await cleanup(0)
}
