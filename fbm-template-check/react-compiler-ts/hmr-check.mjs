// Standalone HMR workflow for the create-vite `react-compiler-ts` template under Vite
// full-bundle mode. This is the offline built-in = template-react (TS) + setupReactCompiler
// (vite.config wires `babel({ presets: [reactCompilerPreset()] })` alongside react()).
//
// No Vite test harness: we spawn the app's own `vite` dev server, drive it with Playwright,
// edit src/App.tsx (which renders the <h1> marker + the counter button), and assert the
// hot update.
//
// react templates use React Fast Refresh, so the correct outcome is a COMPONENT HOT-SWAP
// with STATE PRESERVED (per GOAL.md §1 react-compiler-ts row):
...
async function run() {
  ...
  // 1. Initial render: wait for the starter <h1> marker. Under FBM the server first serves a
  // "Bundling in progress" fallback page (an <h1> placeholder) that auto-reloads when the dev
  // bundle is ready; the baseline (no bundle step) renders the app immediately. Either way we
  // wait for the REAL marker text, not just any <h1>, so we don't assert on the placeholder.
  try {
  ...
  // react (Fast Refresh) => component HOT-SWAP with STATE PRESERVED. The new <h1> text must
  ...
  console.log(
    `PASS ${TAG}: react-compiler-ts hot-swapped App.tsx with counter state preserved.`,
  )
  await cleanup(0)
}
