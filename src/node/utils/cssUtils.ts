...
export async function compileCss(
  ...
  const id = modules
    ? cssModuleRE.test(filepath)
      ? hash_sum(filepath) // Directly importing *.module.css, use filepath to prevent duplicates
      : hash_sum(filename) // <style module> block, use filename (with query) to prevent conflict
    : hash_sum(publicPath)
