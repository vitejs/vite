...
在非常基础的使用层面，使用 Vite 开发和使用一个静态文件服务器并没有太大区别。然而，Vite 还通过原生 ESM 导入提供了许多增益。
...
这在浏览器中将抛出一个错误。Vie 会侦测到这些模块导入的 `.js` 文件并改写他们的路径，例如 `/node_modules/my-dep/dist/my-dep.js?v=1.0.0`，使得浏览器可以正确处理它们。

**依赖缓存**

解析后的依赖请求会通过头文件 `max-age=31536000,immutable` 进行强缓存，以提高在开发期间的页面重载性能。一旦被缓存，这些请求将永远不会再到达开发服务器。如果安装了不同的版本，则附加的版本查询将使它们自动失效。如果您对依赖项进行了手动本地编辑，您可以通过浏览器 devtools 暂时禁用缓存并重新加载页面。

## TypeScript

Vite 支持开箱即用地引入 `.ts` 文件。

Vite 仅执行 `.ts` 文件的翻译工作，并**不**执行任何类型检查。这假设类型检查已经被你的 IDE 或构建过程接管了。（你可以在构建脚本中运行 `tsc --noEmit`）。

Vite 使用 [esbuild](https://github.com/evanw/esbuild) 将 TypeScript 翻译到 JavaScript，约是原始 `tsc` 速度的 20~30 倍，同时 HMR 更新反映到浏览器的时间小于 50ms。

注意因为 `esbuild` 只执行翻译工作而没有类型信息，所以它无需支持特定功能例如常量枚举和隐式 “type-only” 导入。你必须在你的 `tsconfig.json` 中的 `compilerOptions` 里设置 `"isolatedModules": true`，这样 TS 才会警告你哪些功能无法与独立编译模式一同工作。
...
自定义插件还可以自动将 `import React from 'react'` 这类代码注入到每个文件中，以避免手工导入它们。请参阅 [插件 API](./api-plugin) 来了解如何编写这样的插件。

## CSS

导入 `.css` 文件将会在 HMR 支持下把内容插入到 `<style>` 标签中。您还能够以字符串的形式检索已处理的、作为模块的默认导出的 CSS。
...
任何以 `.module.css` 为后缀名的 CSS 文件都被认为是一个 [CSS modules 文件](https://github.com/css-modules/css-modules)。导入这样一个文件会返回一个相应的模块对象：
...
也就是说，Vite 提供了对`.scss`, `.sass`, `.less`, `.styl` 和 `.stylus` 文件的内置支持。没有必要为他们安装特定的 vite 插件，但相应的预处理器本身必须安装其依赖：
...
您还可以通过在文件扩展名前加上 `.module` 来结合使用 CSS 模块和预处理器，例如 `style.module.scss`。
...
预编译的 `.wasm` 文件可以直接被导入 - 默认导出将会是一个返回带着导出 wasm 实例对象 Promise 的初始化函数：
...
这个 `init` 函数也可以将使用传递给 `WebAssembly.instantiate` 的 `imports` 对象，作为其第二个参数：
