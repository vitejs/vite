...
const linePathGlow1 = ref(null)
const linePath2 = ref(null)
const linePathGlow2 = ref(null)
const linePath3 = ref(null)
const linePathGlow3 = ref(null)
const linePath4 = ref(null)
const linePathGlow4 = ref(null)
const linePath5 = ref(null)
const linePathGlow5 = ref(null)
const linePath6 = ref(null)
const linePathGlow6 = ref(null)
const linePath7 = ref(null)
const linePathGlow7 = ref(null)

const inputNode1 = ref(null)

onMounted(() => {
  animateLineGlow(linePathGlow1, inputNode1)
})

const animateLineGlow = (linePathGlow, inputNode = null) => {
  const pathLength = linePathGlow.value.getTotalLength()
  const drawLength = pathLength * 0.06
  const timeline = gsap.timeline()

  gsap.set(linePathGlow.value, {
    strokeDasharray: `${drawLength} ${pathLength - drawLength}`,
    strokeDashoffset: 0
  })

  timeline.to(linePathGlow.value, {
    strokeDashoffset: (pathLength / 2) + ((drawLength) / 2),
    duration: 1.5,
    ease: 'expo.out'
  }, 0)

  if (inputNode) {
    timeline.to(inputNode.value, {
      motionPath: {
        path: linePathGlow.value,
        align: linePathGlow.value,
  ...
  <div class="hero__diagram">
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="844" height="644" viewBox="0 0 844 644" fill="none">
      ...
      <path ref="linePath1"
            ...
            stroke="url(#base_gradient)" :stroke-width="baseStrokeWidth" />

      <!-- Line 1 Glow -->
      <path ref="linePathGlow1"
            d="M843.505 284.659L752.638 284.659C718.596 284.659 684.866 280.049 653.251 271.077L598.822 255.629L0.675021 1.00011"
      ...
      <!-- Line 2 Glow -->
      <path ref="linePathGlow2"
            d="M843.505 298.181L724.342 297.36C708.881 297.36 693.45 296.409 678.22 294.518L598.822 284.659C592.82 284.659 200.538 190.002 0.675028 164.892"
            stroke="url(#glow_gradient)" :stroke-width="baseStrokeWidth" stroke-dasharray="9999"
            stroke-dashoffset="9999" />
      ...
      <defs>
        ...
        <linearGradient id="glow_gradient" x1="88.1032" y1="324.167" x2="843.505" y2="324.167"
                        gradientUnits="userSpaceOnUse">
          <stop offset="0" stop-color="#41D1FF" stop-opacity="0" />
          <stop offset="0.3" stop-color="#41D1FF" stop-opacity="1" />
          <stop offset="0.7" stop-color="#41D1FF" stop-opacity="1" />
