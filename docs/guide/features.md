...
- The glob patterns are treated like import specifiers: they must be either relative (start with `./`) or absolute (start with `/`, resolved relative to project root) or alias path (alias path define in [Vite config](/config/#resolve-alias)).
