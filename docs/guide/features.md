...
By default, the worker script will be emitted as a separate chunk in the production build. When [`worker.format`](/config/worker-options#worker-format) is `'es'`, modules shared between multiple workers - or between a worker and the main build - are deduplicated into a common chunk instead of being bundled into each worker separately; see [`worker.shareChunks`](/config/worker-options#worker-sharechunks) (enabled by default).
