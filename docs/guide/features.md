...
Note that CSS minification will run after PostCSS and will use the [`build.cssTarget`](/config/build-options.md#build-csstarget) option when `build.cssMinify` is `'esbuild'`, or [`css.lightningcss.targets`](/config/shared-options.md#css-lightningcss) when it is `'lightningcss'`.
