...
1. Large dependency that is only used in rare cases or only used in either dev or build should not be statically imported. These are some examples that reduced the config load time by changing static imports to dynamic ones: [vite-plugin-react#212](https://github.com/vitejs/vite-plugin-react/pull/212), [vite-plugin-pwa#224](https://github.com/vite-pwa/vite-plugin-pwa/pull/244).
