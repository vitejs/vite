...
Given that Vite is used by many frameworks, it is purposedly unopinionated about more complex replacements like conditionals. Vite can be extended using [an existent userland plugin](https://github.com/vitejs/awesome-vite#transformers) or a custom plugin that implements the [`transformIndexHtml` hook](./api-plugin#transformindexhtml).
