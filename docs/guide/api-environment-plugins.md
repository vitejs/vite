...
Plugins can add new environments in the `config` hook. For example, [RSC support](/guide/features#react-server-components) uses an additional environment to have a separate module graph with the `react-server` condition:
