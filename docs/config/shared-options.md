...
All preprocessor options also support the `additionalData` option, which can be used to inject extra code for each style content. Note that if you include actual styles and just variables, those styles will be duplicated in the final bundle.
