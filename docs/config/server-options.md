...
The Vite server watcher watches the `root` and skips the `.git/`, `node_modules/`, Playwright's `test-results/`, and Vite's `cacheDir` and `build.outDir` directories by default. When updating a watched file, Vite will apply HMR and update the page only if needed.
