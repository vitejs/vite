...
Plugins that derive additional artifacts from those files, for example pre-compressed copies, should do so after SRI finalization or from written output files.

Vite only emits integrity metadata for JavaScript and CSS assets it can map to final emitted files. Public URL customization, for example through [`base`](/guide/build#public-base-path) or [`experimental.renderBuiltUrl`](/guide/build#advanced-base-options), doesn't change the integrity value as long as Vite can still associate the URL with an emitted JS/CSS asset. Custom preload dependencies or URLs that don't correspond to emitted JS/CSS files are emitted without integrity metadata.
