...
When set to `true`, optimized dependencies that contain .css imports will have the css extracted and bundled in a separate file. This is useful for reducing the number of small css requests coming from component-libraries, etc, replacing it instead with a single large css bundle request per dependency.
