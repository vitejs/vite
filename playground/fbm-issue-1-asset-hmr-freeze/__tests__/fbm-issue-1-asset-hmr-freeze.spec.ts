...
//
// `import pngUrl from './sample.png'` returns a URL STRING. `sample.png` is >4096 B,
// so it takes the EMITTED-asset path (a real hashed `/assets/sample-<hash>.png`), not
// the small-file inline `data:` URI path (the >4096 B path is the one the FBM milestone
// cares about, and the one that froze for CSS `?url` / `.apng` / `.bmp`).
//
// Vite's asset pipeline is EXTENSION-keyed (not content-validating), so `.png` may hold
// any bytes; here it holds KNOWN text content so the spec can assert the served bytes via
// fetch() and edit them with a UNIQUE single-occurrence needle (`PNG-FBM-MARKER-V1`),
...
if (!isBuild) {
  ...
  //    to the asset — a real hashed `/assets/sample-<hash>.png`, NOT an unresolved
  //    `__ROLLDOWN_ASSET__`/`__VITE_ASSET__` placeholder — and fetch(url) returns the
  //    known asset bytes.
  test('.png import resolves to a real served asset URL under FBM (bundledDev)', async () => {
    await expect.poll(() => page.textContent('.app')).toBe('png loaded')

    const url = await page.textContent('.png-url')
    ...
    const fetched = await page.textContent('.png-fetched')
    expect(fetched).toContain('PNG-FBM-MARKER-V1')
    expect(fetched).not.toMatch(/FETCH_FAILED|FETCH_THREW/)
  })

  // 2. HMR (FAILS in FBM — known gap, same as CSS `?url` / `.apng` / `.bmp`): editing the
  //    asset's bytes does NOT update what the emitted URL serves. The emitted asset
  ...
  //    RESULT.md ".png" / "CSS `?url`" / ".apng" — emitted-asset content is not re-emitted
  //    on HMR; #22596 family).
  //
  // Captured empirically under FBM: URL before/after = `/assets/sample-<hash>.png`
  ...
  test.fails('.png serves the UPDATED bytes after an HMR edit under FBM (known fail)', async () => {
    const fetchedBefore = await page.textContent('.png-fetched')
    expect(fetchedBefore).toContain('PNG-FBM-MARKER-V1')

    editFile('sample.png', (code) =>
      code.replace('PNG-FBM-MARKER-V1', 'PNG-FBM-MARKER-V2'),
    )
    ...
    await expect
      .poll(() => page.textContent('.png-fetched'), { timeout: 5000 })
      .toContain('PNG-FBM-MARKER-V2')
  })
}
