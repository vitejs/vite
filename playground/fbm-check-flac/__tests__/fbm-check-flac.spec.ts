...
//
// `.flac` is the SIXTH media entry in `KNOWN_ASSET_TYPES` (constants.ts:169, directly
// after `'wav'` :168 under the `// media` comment :163), handled by the SAME
// extension-keyed asset pipeline as `.wav`/`.mp3`/`.ogg`/`.webm`/`.mp4` and the image
// cluster `.png`/`.jpg`/`.gif`/`.svg`/`.webp`/`.avif`/`.cur`/`.jxl`.
//
// `import flacUrl from './sample.flac'` returns a URL STRING. `sample.flac` is >4096 B,
// so it takes the EMITTED-asset path (a real hashed `/assets/sample-<hash>.flac`), not
// the small-file inline `data:` URI path (the >4096 B path is the one the FBM milestone
// cares about, and the one that froze for CSS `?url` / the image cluster / `.mp4` / `.webm` / `.ogg` / `.mp3` / `.wav`).
//
// Vite's asset pipeline is EXTENSION-keyed (not content-validating), so `.flac` may hold
// any bytes; here it holds KNOWN text content so the spec can assert the served bytes via
// fetch() and edit them with a UNIQUE single-occurrence needle (`FLAC-FBM-MARKER-V1`).
//
...
if (!isBuild) {
  ...
  //    to the asset — a real hashed `/assets/sample-<hash>.flac`, NOT an unresolved
  //    `__ROLLDOWN_ASSET__`/`__VITE_ASSET__` placeholder — and fetch(url) returns the
  //    known asset bytes.
  test('.flac import resolves to a real served asset URL under FBM (bundledDev)', async () => {
    await expect.poll(() => page.textContent('.app')).toBe('flac loaded')

    const url = await page.textContent('.flac-url')
    // Real emitted asset URL (>4096 B → not inlined as a data URI).
    expect(url).toMatch(/\/assets\/sample-[-\w]+\.flac/)
    ...
    const fetched = await page.textContent('.flac-fetched')
    expect(fetched).toContain('FLAC-FBM-MARKER-V1')
    expect(fetched).not.toMatch(/FETCH_FAILED|FETCH_THREW/)
  })

  // 2. HMR (FAILS in FBM — known gap, same as CSS `?url` / the image cluster / `.mp4` / `.webm` / `.ogg` / `.mp3` / `.wav`):
  ...
  //    RESULT.md ".wav" / ".mp3" / ".ogg" / ".webm" / ".mp4" / ".jxl" / ".png" / "CSS `?url`" —
  //    emitted-asset content is not re-emitted on HMR; #22596 family).
  //
  // Expected empirically under FBM: URL before/after = `/assets/sample-<hash>.flac`
  // (UNCHANGED), a fresh fetch returns 200 with the STALE pre-edit bytes, and there is
  // no full reload.
  test.fails('.flac serves the UPDATED bytes after an HMR edit under FBM (known fail)', async () => {
    const fetchedBefore = await page.textContent('.flac-fetched')
    expect(fetchedBefore).toContain('FLAC-FBM-MARKER-V1')

    editFile('sample.flac', (code) =>
      code.replace('FLAC-FBM-MARKER-V1', 'FLAC-FBM-MARKER-V2'),
    )
    ...
    await expect
      .poll(() => page.textContent('.flac-fetched'), { timeout: 5000 })
      .toContain('FLAC-FBM-MARKER-V2')
  })
}
