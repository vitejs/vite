...
//
// `.ogg` is the THIRD media entry in `KNOWN_ASSET_TYPES` (constants.ts:166, directly
// after `'webm'` :165 under the `// media` comment :163), handled by the SAME
// extension-keyed asset pipeline as `.mp4`/`.webm` and the image cluster
// `.png`/`.jpg`/`.gif`/`.svg`/`.webp`/`.avif`/`.cur`/`.jxl`.
//
// `import oggUrl from './sample.ogg'` returns a URL STRING. `sample.ogg` is >4096 B,
// so it takes the EMITTED-asset path (a real hashed `/assets/sample-<hash>.ogg`), not
// the small-file inline `data:` URI path (the >4096 B path is the one the FBM milestone
// cares about, and the one that froze for CSS `?url` / the image cluster / `.mp4` / `.webm`).
//
// Vite's asset pipeline is EXTENSION-keyed (not content-validating), so `.ogg` may hold
// any bytes; here it holds KNOWN text content so the spec can assert the served bytes via
// fetch() and edit them with a UNIQUE single-occurrence needle (`OGG-FBM-MARKER-V1`).
//
...
if (!isBuild) {
  ...
  //    to the asset — a real hashed `/assets/sample-<hash>.ogg`, NOT an unresolved
  //    `__ROLLDOWN_ASSET__`/`__VITE_ASSET__` placeholder — and fetch(url) returns the
  //    known asset bytes.
  test('.ogg import resolves to a real served asset URL under FBM (bundledDev)', async () => {
    await expect.poll(() => page.textContent('.app')).toBe('ogg loaded')

    const url = await page.textContent('.ogg-url')
    // Real emitted asset URL (>4096 B → not inlined as a data URI).
    expect(url).toMatch(/\/assets\/sample-[-\w]+\.ogg/)
    ...
    const fetched = await page.textContent('.ogg-fetched')
    expect(fetched).toContain('OGG-FBM-MARKER-V1')
    expect(fetched).not.toMatch(/FETCH_FAILED|FETCH_THREW/)
  })

  // 2. HMR (FAILS in FBM — known gap, same as CSS `?url` / the image cluster / `.mp4` / `.webm`):
  ...
  //    RESULT.md ".webm" / ".mp4" / ".jxl" / ".png" / "CSS `?url`" — emitted-asset content
  //    is not re-emitted on HMR; #22596 family).
  //
  // Expected empirically under FBM: URL before/after = `/assets/sample-<hash>.ogg`
  ...
  test.fails('.ogg serves the UPDATED bytes after an HMR edit under FBM (known fail)', async () => {
    const fetchedBefore = await page.textContent('.ogg-fetched')
    expect(fetchedBefore).toContain('OGG-FBM-MARKER-V1')

    editFile('sample.ogg', (code) =>
      code.replace('OGG-FBM-MARKER-V1', 'OGG-FBM-MARKER-V2'),
    )
    ...
    await expect
      .poll(() => page.textContent('.ogg-fetched'), { timeout: 5000 })
      .toContain('OGG-FBM-MARKER-V2')
  })
}
