...
// the desired behavior. With the lightningcss transformer (this file), the
// two resolution tests fail on main: sources are relative to the project
// root, so the URL-served map doubles its own path, and the style-tag map —
// whose base is the hosting page — resolves to wrong URLs on any page that
// is not at the root.
// The postcss twin of this file fails the "no filesystem paths" test instead.
