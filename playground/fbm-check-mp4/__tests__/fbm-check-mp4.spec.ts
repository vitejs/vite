...
//
// `.mp4` is the FIRST media entry in `KNOWN_ASSET_TYPES` (constants.ts:164, directly
// after `'jxl'` :161 — the last image entry — under the `// media` comment), handled by
// the SAME extension-keyed asset pipeline as the image cluster
// `.png`/`.jpg`/`.gif`/`.svg`/`.webp`/`.avif`/`.cur`/`.jxl`.
//
// `import mp4Url from './sample.mp4'` returns a URL STRING. `sample.mp4` is >4096 B,
// so it takes the EMITTED-asset path (a real hashed `/assets/sample-<hash>.mp4`), not
...
// `.jfif` / `.pjpeg` / `.pjp` / `.gif` / `.svg` / `.ico` / `.webp` / `.avif` / `.cur` / `.jxl`).
//
// Vite's asset pipeline is EXTENSION-keyed (not content-validating), so `.mp4` may hold
// any bytes; here it holds KNOWN text content so the spec can assert the served bytes via
// fetch() and edit them with a UNIQUE single-occurrence needle (`MP4-FBM-MARKER-V1`).
//
...
if (!isBuild) {
  ...
  //    to the asset — a real hashed `/assets/sample-<hash>.mp4`, NOT an unresolved
  //    `__ROLLDOWN_ASSET__`/`__VITE_ASSET__` placeholder — and fetch(url) returns the
  //    known asset bytes.
  test('.mp4 import resolves to a real served asset URL under FBM (bundledDev)', async () => {
    await expect.poll(() => page.textContent('.app')).toBe('mp4 loaded')

    const url = await page.textContent('.mp4-url')
    // Real emitted asset URL (>4096 B → not inlined as a data URI).
    expect(url).toMatch(/\/assets\/sample-[-\w]+\.mp4/)
    ...
    const fetched = await page.textContent('.mp4-fetched')
    expect(fetched).toContain('MP4-FBM-MARKER-V1')
  ...
  //    `.cur` / `.jxl`): editing the asset's bytes does NOT update what the emitted URL
  ...
  //    the FBM gap (see RESULT.md ".jxl" / ".cur" / ".png" / "CSS `?url`" — emitted-asset
  //    content is not re-emitted on HMR; #22596 family).
  //
  // Expected empirically under FBM: URL before/after = `/assets/sample-<hash>.mp4`
  ...
  test.fails('.mp4 serves the UPDATED bytes after an HMR edit under FBM (known fail)', async () => {
    const fetchedBefore = await page.textContent('.mp4-fetched')
    expect(fetchedBefore).toContain('MP4-FBM-MARKER-V1')

    editFile('sample.mp4', (code) =>
      code.replace('MP4-FBM-MARKER-V1', 'MP4-FBM-MARKER-V2'),
    )
    ...
    await expect
      .poll(() => page.textContent('.mp4-fetched'), { timeout: 5000 })
      .toContain('MP4-FBM-MARKER-V2')
  })
}
