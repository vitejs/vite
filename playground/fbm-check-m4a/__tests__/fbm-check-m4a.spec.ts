...
//
// `.m4a` is the TENTH media entry in `KNOWN_ASSET_TYPES` (constants.ts:173, directly
// after `'mov'` :172 / `'opus'` :171 / `'aac'` :170 under the `// media` comment :163),
// handled by the SAME extension-keyed asset pipeline as `.mov`/`.opus`/`.aac`/`.flac`/`.wav`/`.mp3`/`.ogg`/`.webm`/`.mp4`
// and the image cluster `.png`/`.jpg`/`.gif`/`.svg`/`.webp`/`.avif`/`.cur`/`.jxl`.
//
// `import m4aUrl from './sample.m4a'` returns a URL STRING. `sample.m4a` is >4096 B,
// so it takes the EMITTED-asset path (a real hashed `/assets/sample-<hash>.m4a`), not
// the small-file inline `data:` URI path (the >4096 B path is the one the FBM milestone
// cares about, and the one that froze for CSS `?url` / the image cluster / `.mp4` / `.webm` / `.ogg` / `.mp3` / `.wav` / `.flac` / `.aac` / `.opus` / `.mov`).
//
// Vite's asset pipeline is EXTENSION-keyed (not content-validating), so `.m4a` may hold
// any bytes; here it holds KNOWN text content so the spec can assert the served bytes via
// fetch() and edit them with a UNIQUE single-occurrence needle (`M4A-FBM-MARKER-V1`).
//
...
if (!isBuild) {
  ...
  //    to the asset — a real hashed `/assets/sample-<hash>.m4a`, NOT an unresolved
  //    `__ROLLDOWN_ASSET__`/`__VITE_ASSET__` placeholder — and fetch(url) returns the
  //    known asset bytes.
  test('.m4a import resolves to a real served asset URL under FBM (bundledDev)', async () => {
    await expect.poll(() => page.textContent('.app')).toBe('m4a loaded')

    const url = await page.textContent('.m4a-url')
    // Real emitted asset URL (>4096 B → not inlined as a data URI).
    expect(url).toMatch(/\/assets\/sample-[-\w]+\.m4a/)
    ...
    const fetched = await page.textContent('.m4a-fetched')
    expect(fetched).toContain('M4A-FBM-MARKER-V1')
    expect(fetched).not.toMatch(/FETCH_FAILED|FETCH_THREW/)
  })

  // 2. HMR (FAILS in FBM — known gap, same as CSS `?url` / the image cluster / `.mp4` / `.webm` / `.ogg` / `.mp3` / `.wav` / `.flac` / `.aac` / `.opus` / `.mov`):
  ...
  //    RESULT.md ".mov" / ".opus" / ".aac" / ".flac" / ".wav" / ".mp3" / ".ogg" / ".webm" /
  //    ".mp4" / ".jxl" / ".png" / "CSS `?url`" — emitted-asset content is not re-emitted on HMR; #22596 family).
  //
  // Expected empirically under FBM: URL before/after = `/assets/sample-<hash>.m4a`
  // (UNCHANGED), a fresh fetch returns 200 with the STALE pre-edit bytes, and there is
  // no full reload.
  test.fails('.m4a serves the UPDATED bytes after an HMR edit under FBM (known fail)', async () => {
    const fetchedBefore = await page.textContent('.m4a-fetched')
    expect(fetchedBefore).toContain('M4A-FBM-MARKER-V1')

    editFile('sample.m4a', (code) =>
      code.replace('M4A-FBM-MARKER-V1', 'M4A-FBM-MARKER-V2'),
    )
    ...
    await expect
      .poll(() => page.textContent('.m4a-fetched'), { timeout: 5000 })
      .toContain('M4A-FBM-MARKER-V2')
  })
}
