// Static `.cur` asset imported from JS — mirrors Vite's `playground/assets`
// "asset imports from js" case (index.html: `import url from './nested/asset.png';
// text('.asset-import-relative', url)`). `.cur` is listed in `KNOWN_ASSET_TYPES`
// (constants.ts:160, directly after `'avif'` :159 / before `'jxl'` :161),
// handled by the SAME extension-keyed asset pipeline as
// `.png`/`.jpg`/`.apng`/`.bmp`/`.gif`/`.svg`/`.ico`/`.webp`/`.avif`.
//
// `import curUrl from './sample.cur'` returns a URL STRING pointing at the asset.
// `sample.cur` is >4096 B, so it takes the EMITTED-asset path (a hashed
// `/assets/sample-<hash>.cur`), not the small-file inline `data:` URI path.
//
// Vite's asset pipeline is EXTENSION-keyed, not content-validating, so `.cur` can hold
...
import curUrl from './sample.cur'

// Render the URL value so the spec can assert its shape (real `/assets/...-<hash>.cur`
// vs an unresolved `__ROLLDOWN_ASSET__`/`__VITE_ASSET__` placeholder).
document.querySelector('.cur-url').textContent = curUrl
...
async function renderFetched(href) {
  try {
    ...
    document.querySelector('.cur-fetched').textContent = body
  } catch (e) {
    document.querySelector('.cur-fetched').textContent = `FETCH_THREW ${e}`
  }
}
renderFetched(curUrl)

document.querySelector('.app').textContent = 'cur loaded'
...
if (import.meta.hot) {
  import.meta.hot.accept('./sample.cur', (mod) => {
    if (mod) {
      document.querySelector('.cur-url').textContent = mod.default
