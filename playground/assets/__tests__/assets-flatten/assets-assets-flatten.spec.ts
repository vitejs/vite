...
const assetMatch = isBuild
  ? /\/foo\/bar\/assets\/nested\/asset-[-\w]{8}\.png/
  : '/foo/bar/nested/asset.png'

const encodedAssetMatch = isBuild
  ? /\/foo\/bar\/assets\/nested\/asset_small_-[-\w]{8}\.png/
...
describe('image', () => {
  test('src', async () => {
    ...
    expect(src).toMatch(
      isBuild
        ? /\/foo\/bar\/assets\/nested\/html-only-asset-[-\w]{8}\.jpg/
  ...
  test('srcset', async () => {
    ...
    srcset.split(', ').forEach((s) => {
      expect(s).toMatch(
        isBuild
          ? /\/foo\/bar\/assets\/nested\/asset-[-\w]{8}\.png \dx/
...
test('new URL(`./${dynamic}?abc`, import.meta.url)', async () => {
  ...
  expect(await page.textContent('.dynamic-import-meta-url-2-query')).toMatch(
    isBuild
      ? /\/foo\/bar\/assets\/nested\/asset-[-\w]{8}\.png\?abc/
...
test.runIf(isBuild)('manifest', async () => {
  ...
  for (const file of listAssets('foo')) {
    if (file.endsWith('.css')) {
      ...
      expect(entry.assets, file).toContain(`assets/${file}`)
    }
  }
})
...
test.runIf(isBuild)('assets inside <noscript> is rewrote', async () => {
  const indexHtml = readFile('./dist/assets-flatten/index.html')
  expect(indexHtml).toMatch(
    /<img class="noscript" src="\/foo\/bar\/assets\/nested\/asset-[-\w]+\.png" \/>/,
  )
})
