...
//
// `.aac` is the SEVENTH media entry in `KNOWN_ASSET_TYPES` (constants.ts:170, directly
// after `'flac'` :169 under the `// media` comment :163), handled by the SAME
// extension-keyed asset pipeline as `.flac`/`.wav`/`.mp3`/`.ogg`/`.webm`/`.mp4` and the
// image cluster `.png`/`.jpg`/`.gif`/`.svg`/`.webp`/`.avif`/`.cur`/`.jxl`.
//
// `import aacUrl from './sample.aac'` returns a URL STRING. `sample.aac` is >4096 B,
// so it takes the EMITTED-asset path (a real hashed `/assets/sample-<hash>.aac`), not
// the small-file inline `data:` URI path (the >4096 B path is the one the FBM milestone
// cares about, and the one that froze for CSS `?url` / the image cluster / `.mp4` / `.webm` / `.ogg` / `.mp3` / `.wav` / `.flac`).
//
// Vite's asset pipeline is EXTENSION-keyed (not content-validating), so `.aac` may hold
// any bytes; here it holds KNOWN text content so the spec can assert the served bytes via
// fetch() and edit them with a UNIQUE single-occurrence needle (`AAC-FBM-MARKER-V1`).
//
...
if (!isBuild) {
  ...
  //    to the asset — a real hashed `/assets/sample-<hash>.aac`, NOT an unresolved
  //    `__ROLLDOWN_ASSET__`/`__VITE_ASSET__` placeholder — and fetch(url) returns the
  //    known asset bytes.
  test('.aac import resolves to a real served asset URL under FBM (bundledDev)', async () => {
    await expect.poll(() => page.textContent('.app')).toBe('aac loaded')

    const url = await page.textContent('.aac-url')
    // Real emitted asset URL (>4096 B → not inlined as a data URI).
    expect(url).toMatch(/\/assets\/sample-[-\w]+\.aac/)
    ...
    const fetched = await page.textContent('.aac-fetched')
    expect(fetched).toContain('AAC-FBM-MARKER-V1')
    expect(fetched).not.toMatch(/FETCH_FAILED|FETCH_THREW/)
  })

  // 2. HMR (FAILS in FBM — known gap, same as CSS `?url` / the image cluster / `.mp4` / `.webm` / `.ogg` / `.mp3` / `.wav` / `.flac`):
  ...
  //    RESULT.md ".flac" / ".wav" / ".mp3" / ".ogg" / ".webm" / ".mp4" / ".jxl" / ".png" /
  //    "CSS `?url`" — emitted-asset content is not re-emitted on HMR; #22596 family).
  //
  // Expected empirically under FBM: URL before/after = `/assets/sample-<hash>.aac`
  // (UNCHANGED), a fresh fetch returns 200 with the STALE pre-edit bytes, and there is
  // no full reload.
  test.fails('.aac serves the UPDATED bytes after an HMR edit under FBM (known fail)', async () => {
    const fetchedBefore = await page.textContent('.aac-fetched')
    expect(fetchedBefore).toContain('AAC-FBM-MARKER-V1')

    editFile('sample.aac', (code) =>
      code.replace('AAC-FBM-MARKER-V1', 'AAC-FBM-MARKER-V2'),
    )
    ...
    await expect
      .poll(() => page.textContent('.aac-fetched'), { timeout: 5000 })
      .toContain('AAC-FBM-MARKER-V2')
  })
}
