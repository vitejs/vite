...
//
// `.mp3` is the FOURTH media entry in `KNOWN_ASSET_TYPES` (constants.ts:167, directly
// after `'ogg'` :166 under the `// media` comment :163), handled by the SAME
// extension-keyed asset pipeline as `.ogg`/`.webm`/`.mp4` and the image cluster
// `.png`/`.jpg`/`.gif`/`.svg`/`.webp`/`.avif`/`.cur`/`.jxl`.
//
// `import mp3Url from './sample.mp3'` returns a URL STRING. `sample.mp3` is >4096 B,
// so it takes the EMITTED-asset path (a real hashed `/assets/sample-<hash>.mp3`), not
// the small-file inline `data:` URI path (the >4096 B path is the one the FBM milestone
// cares about, and the one that froze for CSS `?url` / the image cluster / `.mp4` / `.webm` / `.ogg`).
//
// Vite's asset pipeline is EXTENSION-keyed (not content-validating), so `.mp3` may hold
// any bytes; here it holds KNOWN text content so the spec can assert the served bytes via
// fetch() and edit them with a UNIQUE single-occurrence needle (`MP3-FBM-MARKER-V1`).
//
...
if (!isBuild) {
  ...
  //    to the asset — a real hashed `/assets/sample-<hash>.mp3`, NOT an unresolved
  //    `__ROLLDOWN_ASSET__`/`__VITE_ASSET__` placeholder — and fetch(url) returns the
  //    known asset bytes.
  test('.mp3 import resolves to a real served asset URL under FBM (bundledDev)', async () => {
    await expect.poll(() => page.textContent('.app')).toBe('mp3 loaded')

    const url = await page.textContent('.mp3-url')
    // Real emitted asset URL (>4096 B → not inlined as a data URI).
    expect(url).toMatch(/\/assets\/sample-[-\w]+\.mp3/)
    ...
    const fetched = await page.textContent('.mp3-fetched')
    expect(fetched).toContain('MP3-FBM-MARKER-V1')
    expect(fetched).not.toMatch(/FETCH_FAILED|FETCH_THREW/)
  })

  // 2. HMR (FAILS in FBM — known gap, same as CSS `?url` / the image cluster / `.mp4` / `.webm` / `.ogg`):
  ...
  //    RESULT.md ".ogg" / ".webm" / ".mp4" / ".jxl" / ".png" / "CSS `?url`" — emitted-asset
  //    content is not re-emitted on HMR; #22596 family).
  //
  // Expected empirically under FBM: URL before/after = `/assets/sample-<hash>.mp3`
  // (UNCHANGED), a fresh fetch returns 200 with the STALE pre-edit bytes, and there is
  // no full reload.
  test.fails('.mp3 serves the UPDATED bytes after an HMR edit under FBM (known fail)', async () => {
    const fetchedBefore = await page.textContent('.mp3-fetched')
    expect(fetchedBefore).toContain('MP3-FBM-MARKER-V1')

    editFile('sample.mp3', (code) =>
      code.replace('MP3-FBM-MARKER-V1', 'MP3-FBM-MARKER-V2'),
    )
    ...
    await expect
      .poll(() => page.textContent('.mp3-fetched'), { timeout: 5000 })
      .toContain('MP3-FBM-MARKER-V2')
  })
}
