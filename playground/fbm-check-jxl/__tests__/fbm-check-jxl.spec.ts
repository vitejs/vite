...
//
// `.jxl` is in `KNOWN_ASSET_TYPES` (constants.ts:161, directly after `'cur'` :160 —
// the last image entry), handled by the SAME extension-keyed asset pipeline as
// `.png`/`.jpg`/`.apng`/`.bmp`/`.gif`/`.svg`/`.ico`/`.webp`/`.avif`/`.cur`.
//
// `import jxlUrl from './sample.jxl'` returns a URL STRING. `sample.jxl` is >4096 B,
// so it takes the EMITTED-asset path (a real hashed `/assets/sample-<hash>.jxl`), not
...
// `.jfif` / `.pjpeg` / `.pjp` / `.gif` / `.svg` / `.ico` / `.webp` / `.avif` / `.cur`).
//
// Vite's asset pipeline is EXTENSION-keyed (not content-validating), so `.jxl` may hold
// any bytes; here it holds KNOWN text content so the spec can assert the served bytes via
// fetch() and edit them with a UNIQUE single-occurrence needle (`JXL-FBM-MARKER-V1`),
...
if (!isBuild) {
  ...
  //    to the asset — a real hashed `/assets/sample-<hash>.jxl`, NOT an unresolved
  //    `__ROLLDOWN_ASSET__`/`__VITE_ASSET__` placeholder — and fetch(url) returns the
  //    known asset bytes.
  test('.jxl import resolves to a real served asset URL under FBM (bundledDev)', async () => {
    await expect.poll(() => page.textContent('.app')).toBe('jxl loaded')

    const url = await page.textContent('.jxl-url')
    // Real emitted asset URL (>4096 B → not inlined as a data URI).
    expect(url).toMatch(/\/assets\/sample-[-\w]+\.jxl/)
    ...
    const fetched = await page.textContent('.jxl-fetched')
    expect(fetched).toContain('JXL-FBM-MARKER-V1')
  ...
  //    `.cur`): editing the asset's bytes does NOT update what the emitted URL serves. The
  ...
  //    (see RESULT.md ".cur" / ".avif" / ".webp" / ".ico" / ".gif" / ".png" / "CSS `?url`" —
  //    emitted-asset content is not re-emitted on HMR; #22596 family).
  //
  // Expected empirically under FBM: URL before/after = `/assets/sample-<hash>.jxl`
  ...
  test.fails('.jxl serves the UPDATED bytes after an HMR edit under FBM (known fail)', async () => {
    const fetchedBefore = await page.textContent('.jxl-fetched')
    expect(fetchedBefore).toContain('JXL-FBM-MARKER-V1')

    editFile('sample.jxl', (code) =>
      code.replace('JXL-FBM-MARKER-V1', 'JXL-FBM-MARKER-V2'),
    )
    ...
    await expect
      .poll(() => page.textContent('.jxl-fetched'), { timeout: 5000 })
      .toContain('JXL-FBM-MARKER-V2')
  })
}
