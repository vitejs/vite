...
describe('tsconfig-json.spec.ts', () => {
  before(mochaSetup)
  after(mochaReset)

  it('should respected each `tsconfig.json`s compilerOptions', () => {
  ...
  describe('transformWithEsbuild', () => {
    it('merge tsconfigRaw object', async () => {
    ...
    it('overwrite tsconfigRaw string', async () => {
      const main = path.resolve(__dirname, '../src/main.ts')
      const mainContent = fs.readFileSync(main, 'utf-8')
      const result = await transformWithEsbuild(mainContent, main, {
        tsconfigRaw: `{
