...
describe('vue.spec.ts', () => {
  ...
  it('template/script latest syntax support', async () => {
    expect(await page.textContent('.syntax')).toBe('baz')
  })

  it('should remove comments in prod', async () => {
    expect(await page.innerHTML('.comments')).toBe(
      isBuild ? `` : `<!--hello-->`
    )
  })
  ...
  describe('dep scan', () => {
    it('scan deps from <script setup lang="ts">', async () => {
      expect(await page.textContent('.scan')).toBe('ok')
    })

    it('find deps on initial scan', () => {
  ...
  describe('css modules', () => {
    ...
    it('with preprocessor + name', async () => {
  ...
  describe('asset reference', () => {
    ...
    it('absolute import from public dir', async () => {
  ...
  describe('hmr', () => {
    it('should re-render and preserve state when template is edited', async () => {
      editFile('Hmr.vue', (code) => code.replace('HMR', 'HMR updated'))
      await untilUpdated(() => page.textContent('h2.hmr'), 'HMR updated')
      expect(await page.textContent('.hmr-inc')).toMatch('count is 1')
    })

    it('should update style and preserve state when style is edited', async () => {
    ...
    it('should reload and reset state when script is edited', async () => {
    ...
    it('global hmr for some scenarios', async () => {
      editFile('Hmr.vue', (code) =>
    ...
    it('should re-render when template is emptied', async () => {
      editFile('Hmr.vue', () => '')
  ...
  describe('src imports', () => {
    ...
    it('tempalte src import hmr', async () => {
      const el = await page.$('.src-imports-style')
