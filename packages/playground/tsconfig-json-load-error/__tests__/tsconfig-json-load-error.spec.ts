...
describe('tsconfig-json-load-error.spec.ts', () => {
  before(mochaSetup)
  after(mochaReset)

  if (isBuild) {
    it('should throw an error on build', () => {
    ...
    it('should not output files to dist', () => {
      let err
      try {
    ...
    it('should log 500 error in browser for malformed tsconfig', () => {
    ...
    it('should show error overlay for tsconfig error', async () => {
    ...
    it('should reload when tsconfig is changed', async () => {
      await editFile('has-error/tsconfig.json', (content) => {
        return content.replace('"compilerOptions":', '"compilerOptions":{}')
      })
      await untilUpdated(() => {
        return browserLogs.find(
          (x) => x === 'tsconfig error fixed, file loaded'
        )
