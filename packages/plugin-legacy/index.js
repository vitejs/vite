...
function viteLegacyPlugin(options = {}) {
  ...
  if (legacyCodeExecutionFormatUsable.includes(options.legacyExecutionFormat)) {
  ...
  const DEFAULT_LEGACY_POLYFILL_FOR_SYSTEM_JS = [
  ...
  const legacyPostPlugin = {
    ...
    configResolved(_config) {
      ...
      const createLegacyOutput = (options = {}) => {
        return {
          ...options,
          format: legacyCodeExecutionFormat,
    ...
    transformIndexHtml(html, { chunk }) {
      ...
      if (legacyEntryFilename) {
        const src = config.base + legacyEntryFilename
...
async function buildPolyfillChunk(
  ...
  if (codeChunksNames.length == 1 && mergePolyfillWithLegacyCode) {
    ...
    bundle[codeChunksNames[0]].code = polyfillChunk.code + appCode
  } else {
