...
It may be desirable to recreate inline scripts generated by the legacy plugin, e.g. if Vite is not being used to generate your HTML.

These values can be retrieved via

```js
const { safari10NoModuleFixCode, legacyInlineScriptId, legacyInlineScriptCode } = require('@vitejs/plugin-legacy')
```
