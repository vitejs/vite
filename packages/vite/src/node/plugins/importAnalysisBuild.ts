...
export const preloadMethod = `__preloadDeps` + randomHash()
export const preloadHelperId = '\0vite/preload-helper.js'

const chunkRegistryPlaceholder = '__vite__chunkRegistry' + randomHash();
    ...
    generateBundle({ format }, bundle) {
            ...
            (function traverseChunkDependencies (chunkName: string) {
              ...
              if (chunk) {
                ...
                if (chunk.type === 'chunk') {
                  ...
                  chunk.viteMetadata!.importedCss.forEach((file) => dependencies.add(file))
                }
              } else {
                const removedPureCssFiles = removedPureCssFilesCache.get(config)!
      ...
      if (chunkToPatchWithRegistry) {
        ...
        s.overwrite(
          ...
          chunkToPatchWithRegistry.code.indexOf(chunkRegistryPlaceholder) + chunkRegistryPlaceholder.length,
          fileDepsCode,
        )
